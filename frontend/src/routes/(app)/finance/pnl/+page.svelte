...
<script lang="ts">
	...
	const calculations = $derived({
		...
		grossMargin: (
			((currentData.revenue.total - currentData.cogs.total) / currentData.revenue.total) *
			100
		).toFixed(1),
		ebitda: currentData.revenue.total - currentData.cogs.total - currentData.opex.total,
		ebitdaMargin: (
			((currentData.revenue.total - currentData.cogs.total - currentData.opex.total) /
				currentData.revenue.total) *
			100
		).toFixed(1),
		ebit:
			currentData.revenue.total -
			currentData.cogs.total -
			currentData.opex.total -
			currentData.depreciation,
		netIncome:
			currentData.revenue.total -
			currentData.cogs.total -
			currentData.opex.total -
	...
	<header class="page-header">
		<div>
			...
			<p class="page-subtitle">
				Standardized income statement showing revenues, expenses, margins, and bottom line
			</p>
		</div>
...
<style>
	...
	.summary-val.text-warning {
		color: var(--color-warning);
	}
	.summary-val.text-accent {
	...
	.section-row.margin-row {
		font-weight: 500;
		color: var(--color-text-tertiary);
		font-size: 0.75rem;
		font-style: italic;
	}
	.section-row.margin-row .row-val {
		color: var(--color-text-secondary);
		font-weight: 600;
	}

	.section-row.net-income-row {
		font-weight: 800;
		color: var(--color-accent);
