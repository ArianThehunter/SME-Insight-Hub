...
<script lang="ts">
	...
	function exportCashflowCSV() {
		...
		const rows = cashFlowData
			.map((d) => `"${d.month}",${d.inflow},${d.outflow},${d.inflow - d.outflow},${d.balance}`)
			.join('\n');
	...
	const metrics = {
		...
		growthRate: Math.round(
			((currentMonth.inflow - currentMonth.outflow - (prevMonth.inflow - prevMonth.outflow)) /
				(prevMonth.inflow - prevMonth.outflow)) *
				100
		),
		runway: 24 // months
	};
	...
	const balancePath = cashFlowData
		.map((d, i) => `${i === 0 ? 'M' : 'L'} ${getX(i)} ${getY(d.balance / 2)}`)
		.join(' ');
</script>
	...
	<header class="page-header">
		<div>
			...
			<p class="page-subtitle">
				Track liquid assets, cash runway, inflows, and operational cash burns
			</p>
		</div>
	...
	<div class="summary-grid">
		...
		<div class="summary-card">
			<span class="summary-val text-success"
				>+{metrics.growthRate}% <TrendingUp size={16} class="inline-icon" /></span
			>
			...
			<div class="chart-container">
				<svg viewBox="0 0 {chartWidth} {chartHeight}" class="svg-chart">
					...
					{#each cashFlowData as d, i}
						...
						/>
						<!-- Month Labels -->
						<text {x} y={chartHeight - 8} class="chart-axis-label text-center"
							>{d.month.split(' ')[0]}</text
						>
					{/each}
				</svg>
			</div>
	...
	<div class="details-grid animate-fade-in-up">
		<div class="details-card">
			...
			<ul class="breakdown-list">
				<li>
					<span class="label">Product Sales (B2C + Retail)</span> <span class="val">৳ 480,000</span>
				</li>
				<li>
					<span class="label">B2B Wholesale Accounts</span> <span class="val">৳ 190,000</span>
				</li>
				<li>
					<span class="label">Direct Customer Pre-orders</span> <span class="val">৳ 50,000</span>
				</li>
			</ul>
		</div>
		...
		<div class="details-card">
			...
			<ul class="breakdown-list">
				<li>
					<span class="label">Inventory & Material Procurement</span>
					<span class="val">৳ 380,000</span>
				</li>
				<li>
					<span class="label">Rent, Power & Overheads</span> <span class="val">৳ 85,000</span>
				</li>
				<li>
					<span class="label">Salaries & Contractor Fees</span> <span class="val">৳ 62,000</span>
				</li>
...
<style>
	...
	.details-title.text-success {
		color: var(--color-success);
	}
	.details-title.text-danger {
