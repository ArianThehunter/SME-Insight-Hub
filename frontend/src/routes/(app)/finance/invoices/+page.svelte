...
<script lang="ts">
	...
	function exportInvoicesCSV() {
		const headers =
			'Invoice_ID,Customer_Name,Subtotal_BDT,VAT_15pct_BDT,Total_Amount_BDT,Issue_Date,Due_Date,Status\n';
		const rows = filtered
			.map(
				(inv) =>
	...
	<header class="page-header">
		<div>
			...
			<p class="page-subtitle">
				Generate client invoices, track collections, calculate 15% VAT, and follow up on overdue
				bills
			</p>
		</div>
		...
		<div class="modal-backdrop" onclick={() => (selectedInvoice = null)} role="presentation">
			...
			>
				...
				<div class="modal-body">
					<div class="detail-grid">
						<div class="detail-item">
							<span class="d-lbl">Issue Date</span><span class="d-val"
								>{selectedInvoice.issuedDate}</span
							>
						</div>
						<div class="detail-item">
							<span class="d-lbl">Payment Due</span><span class="d-val"
								>{selectedInvoice.dueDate}</span
							>
						</div>
						<div class="detail-item">
							<span class="d-lbl">Subtotal (Pre-tax)</span><span class="d-val"
								>৳ {selectedInvoice.subtotal.toLocaleString()}</span
							>
						</div>
						<div class="detail-item">
							<span class="d-lbl">VAT Mushak 6.3 (15%)</span><span class="d-val"
								>৳ {selectedInvoice.tax.toLocaleString()}</span
							>
						</div>
						<div class="detail-item">
							<span class="d-lbl">Total Payable</span><span class="d-val text-accent font-bold"
								>৳ {selectedInvoice.amount.toLocaleString()}</span
							>
						</div>
						<div class="detail-item">
							<span class="d-lbl">Current Status</span><span
								class="badge badge-{statusClass(selectedInvoice.status)}"
				...
				<div class="modal-footer">
					...
					<button class="btn-primary" onclick={() => (selectedInvoice = null)}> Done </button>
				</div>
			</div>
		</div>
	{/if}
				...
				<form onsubmit={handleCreateInvoice} class="modal-form">
					<div class="form-group">
						...
						<input
							id="ai-cust"
							type="text"
							placeholder="e.g. Sylhet Tea Gardens"
