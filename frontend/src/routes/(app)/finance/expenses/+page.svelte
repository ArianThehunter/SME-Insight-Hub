...
<script lang="ts">
	import {
		Search, Plus, Download, DollarSign, Wallet, FileText,
		ArrowRightLeft, TrendingUp, X, Eye, CheckCircle2
	} from '@lucide/svelte';
	...
	let expenseList = $state<ExpenseItem[]>([
		{ id: 'EXP-001', desc: 'Banani Office Rent (August)', category: 'Rent & Utilities', amount: 85000, date: '2026-08-01', method: 'Bank Transfer', status: 'Paid', vendor: 'Building Management' },
		{ id: 'EXP-002', desc: 'DESCO Electricity Utility Bill', category: 'Rent & Utilities', amount: 12500, date: '2026-08-05', method: 'MFS (bKash)', status: 'Paid', vendor: 'DESCO' },
		{ id: 'EXP-003', desc: 'Cotton Yarn & Fabric Bulk Purchase', category: 'Cost of Goods Sold', amount: 120000, date: '2026-08-08', method: 'Bank Transfer', status: 'Paid', vendor: 'Narayanganj Textile Mills' },
		{ id: 'EXP-004', desc: 'Office Supplies & A4 Paper', category: 'Operations', amount: 3200, date: '2026-08-10', method: 'Cash', status: 'Paid', vendor: 'Ananya Stationers' },
		{ id: 'EXP-005', desc: 'Delivery Van Fuel & Maintenance', category: 'Logistics', amount: 8500, date: '2026-08-12', method: 'Card', status: 'Paid', vendor: 'Padma Oil' },
		{ id: 'EXP-006', desc: 'Digital Ads & Meta Campaigns', category: 'Marketing', amount: 15000, date: '2026-08-15', method: 'Card', status: 'Pending', vendor: 'Meta Platforms' },
		{ id: 'EXP-007', desc: 'Staff Training & Skill Workshop', category: 'Operations', amount: 22000, date: '2026-08-16', method: 'Bank Transfer', status: 'Paid', vendor: 'BRAC Training' },
		{ id: 'EXP-008', desc: 'Eco Packaging Boxes (1000 pcs)', category: 'Logistics', amount: 22000, date: '2026-08-18', method: 'Bank Transfer', status: 'Pending', vendor: 'Packaging BD' },
	]);
	...
	const summary = $derived({
		...
		totalPending: expenseList.filter(e => e.status === 'Pending').reduce((acc, e) => acc + e.amount, 0),
		total: expenseList.reduce((acc, e) => acc + e.amount, 0),
	});

	const categoryBreakdown = $derived(
		Array.from(new Set(expenseList.map(e => e.category))).map(cat => {
			const total = expenseList.filter(e => e.category === cat).reduce((acc, e) => acc + e.amount, 0);
			return { name: cat, amount: total, percent: Math.round(total / (summary.total || 1) * 100) };
	...
	function exportExpensesCSV() {
		const headers = 'Expense_ID,Description,Category,Amount_BDT,Date,Payment_Method,Status,Vendor\n';
...
<div class="page animate-fade-in">
	<header class="page-header">
		<div>
			...
			<p class="page-subtitle">Track overheads, operational vendor bills, procurement costs, and payment approvals</p>
		</div>
	...
	<div class="toolbar">
		<div class="search-box">
			<Search size={16} />
			<input type="text" bind:value={searchQuery} placeholder="Search expense description or ID..." class="search-input" aria-label="Search expenses" />
		</div>
		<div class="toolbar-right">
			...
			<button class="btn-icon" onclick={exportExpensesCSV} title="Export CSV" aria-label="Export expenses CSV">
				<Download size={16} />
			</button>
		</div>
	</div>
				...
				<tbody>
					{#each filtered as e}
						<tr>
							...
							<td>{e.method}</td>
							<td>
								<span class="badge badge-{e.status === 'Paid' ? 'success' : 'warning'}">{e.status}</span>
							</td>
							<td>
								<button class="btn-icon-row" onclick={() => selectedExpense = e} title="View expense details">
									<Eye size={14} />
								</button>
							</td>
						</tr>
	...
	{#if selectedExpense}
		<div class="modal-backdrop" onclick={() => selectedExpense = null} role="presentation">
			<div class="modal-card" onclick={(e) => e.stopPropagation()} onkeydown={(e) => e.stopPropagation()} role="dialog" aria-labelledby="exp-details-title" tabindex="-1">
				<div class="modal-header">
					<div>
						<h2 id="exp-details-title" class="modal-title">{selectedExpense.desc}</h2>
						<span class="exp-tag">{selectedExpense.id}</span>
					</div>
					<button class="btn-close" onclick={() => selectedExpense = null} aria-label="Close dialog">
						<X size={18} />
					</button>
				</div>
				<div class="modal-body">
					<div class="detail-grid">
						<div class="detail-item"><span class="d-lbl">Category</span><span class="d-val">{selectedExpense.category}</span></div>
						<div class="detail-item"><span class="d-lbl">Disbursed Amount</span><span class="d-val text-accent font-bold">৳ {selectedExpense.amount.toLocaleString()}</span></div>
						<div class="detail-item"><span class="d-lbl">Payment Date</span><span class="d-val">{selectedExpense.date}</span></div>
						<div class="detail-item"><span class="d-lbl">Payment Method</span><span class="d-val">{selectedExpense.method}</span></div>
						<div class="detail-item"><span class="d-lbl">Vendor / Payee</span><span class="d-val">{selectedExpense.vendor || 'Direct Payee'}</span></div>
						<div class="detail-item"><span class="d-lbl">Payment Status</span><span class="badge badge-{selectedExpense.status === 'Paid' ? 'success' : 'warning'}">{selectedExpense.status}</span></div>
					</div>
				</div>
				<div class="modal-footer">
					<button class="btn-primary" onclick={() => selectedExpense = null}>
						Done
					</button>
				</div>
			</div>
	...
	{#if showCreateModal}
		<div class="modal-backdrop" onclick={() => showCreateModal = false} role="presentation">
			<div class="modal-card" onclick={(e) => e.stopPropagation()} onkeydown={(e) => e.stopPropagation()} role="dialog" aria-labelledby="add-exp-title" tabindex="-1">
				<div class="modal-header">
				...
				<form onsubmit={handleLogExpense} class="modal-form">
					<div class="form-group">
						...
						<input id="ae-desc" type="text" placeholder="e.g. WASA Water Bill & Maintenance" bind:value={newDesc} required />
					</div>
					<div class="form-row">
						<div class="form-group">
						...
						<div class="form-group">
							...
							<input id="ae-amt" type="number" placeholder="5000" bind:value={newAmount} min="1" required />
						</div>
					</div>
					<div class="form-row">
						<div class="form-group">
...
<style>
	...
	.text-success .summary-val { color: var(--color-success); }
	.text-warning .summary-val { color: #d97706; }

	.breakdown-card { padding: 18px; display: flex; flex-direction: column; gap: 12px; }
	...
	.b-head { display: flex; justify-content: space-between; font-size: 0.75rem; color: var(--color-text-secondary); }
	...
	.exp-tag { font-family: monospace; font-size: 0.75rem; background: var(--color-bg-primary); padding: 2px 6px; border-radius: 4px; border: 1px solid var(--color-border); color: var(--color-accent); }
