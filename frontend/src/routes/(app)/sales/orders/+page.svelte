...
<script lang="ts">
	import {
		Search, Filter, Download, Plus, Eye, ChevronLeft, ChevronRight,
		X, CheckCircle2, ShoppingCart, Clock, Truck, Ban, Printer
	} from '@lucide/svelte';
	...
	let ordersList = $state<Order[]>([
		{ id: 'ORD-2026-001', customer: 'Rahman Textiles Ltd.', date: '2026-06-18', amount: 245000, status: 'completed', payment: 'paid', items: 12, deliveryAddress: 'Dilkusha C/A, Dhaka' },
		{ id: 'ORD-2026-002', customer: 'Dhaka Electronics Hub', date: '2026-06-17', amount: 182500, status: 'processing', payment: 'partial', items: 8, deliveryAddress: 'Elephant Road, Dhaka' },
		{ id: 'ORD-2026-003', customer: 'Chittagong Spice Co.', date: '2026-06-17', amount: 67800, status: 'completed', payment: 'paid', items: 5, deliveryAddress: 'Khatunganj, Chittagong' },
		{ id: 'ORD-2026-004', customer: 'Sylhet Tea Gardens', date: '2026-06-16', amount: 425000, status: 'shipped', payment: 'paid', items: 20, deliveryAddress: 'Sreemangal, Sylhet' },
		{ id: 'ORD-2026-005', customer: 'Rajshahi Mangoes Inc.', date: '2026-06-16', amount: 38200, status: 'pending', payment: 'unpaid', items: 3, deliveryAddress: 'Shaheb Bazar, Rajshahi' },
		{ id: 'ORD-2026-006', customer: 'Khulna Fisheries', date: '2026-06-15', amount: 156000, status: 'completed', payment: 'paid', items: 14, deliveryAddress: 'Rupsha, Khulna' },
		{ id: 'ORD-2026-007', customer: 'Narayanganj Jute Works', date: '2026-06-15', amount: 890000, status: 'processing', payment: 'partial', items: 35, deliveryAddress: 'Tanbazar, Narayanganj' },
		{ id: 'ORD-2026-008', customer: 'Comilla Ceramics', date: '2026-06-14', amount: 72400, status: 'cancelled', payment: 'refunded', items: 6, deliveryAddress: 'EPZ Road, Comilla' },
		{ id: 'ORD-2026-009', customer: 'Bogura Steel Corp.', date: '2026-06-14', amount: 1250000, status: 'shipped', payment: 'paid', items: 45, deliveryAddress: 'Santahar Road, Bogura' },
		{ id: 'ORD-2026-010', customer: 'Gazipur Garments Ltd.', date: '2026-06-13', amount: 324000, status: 'completed', payment: 'paid', items: 18, deliveryAddress: 'Konabari, Gazipur' },
		{ id: 'ORD-2026-011', customer: 'Jessore Food Products', date: '2026-06-13', amount: 95600, status: 'pending', payment: 'unpaid', items: 7, deliveryAddress: 'Chowrasta, Jessore' },
		{ id: 'ORD-2026-012', customer: 'Mymensingh Dairy Farm', date: '2026-06-12', amount: 145200, status: 'completed', payment: 'paid', items: 10, deliveryAddress: 'Valuka, Mymensingh' },
		{ id: 'ORD-2026-013', customer: 'Rangpur Agro Solutions', date: '2026-06-12', amount: 518000, status: 'processing', payment: 'partial', items: 22, deliveryAddress: 'Station Road, Rangpur' },
		{ id: 'ORD-2026-014', customer: 'Barishal Marine Co.', date: '2026-06-11', amount: 267000, status: 'shipped', payment: 'paid', items: 15, deliveryAddress: 'Launch Ghat, Barishal' },
		{ id: 'ORD-2026-015', customer: 'Tangail Handicrafts', date: '2026-06-11', amount: 43500, status: 'completed', payment: 'paid', items: 4, deliveryAddress: 'Porabari, Tangail' },
	]);
	...
	function exportOrdersCSV() {
		const headers = 'Order_ID,Customer,Order_Date,Amount_BDT,Items_Count,Status,Payment_Status,Address\n';
...
<div class="page">
	...
	<div class="toolbar animate-fade-in">
		<div class="search-box">
			<Search size={16} />
			<input type="text" bind:value={searchQuery} placeholder="Search orders by ID or customer..." class="search-input" aria-label="Search orders" />
		</div>
		<div class="toolbar-right">
			...
			<button class="btn-icon" onclick={exportOrdersCSV} title="Export CSV" aria-label="Export orders CSV">
				<Download size={16} />
			</button>
		</div>
	</div>
	...
	<div class="table-card animate-fade-in-up">
				...
				<tbody>
					{#each paginatedOrders as o}
						<tr>
							...
							<td>
								<button class="btn-icon-row" onclick={() => selectedOrder = o} title="View order details">
									<Eye size={14} />
								</button>
							</td>
						</tr>
		...
		<div class="pagination">
			<span class="page-info">Showing {(currentPage - 1) * perPage + 1}–{Math.min(currentPage * perPage, filteredOrders.length)} of {filteredOrders.length} orders</span>
	...
	{#if selectedOrder}
		<div class="modal-backdrop" onclick={() => selectedOrder = null} role="presentation">
			<div class="modal-card" onclick={(e) => e.stopPropagation()} onkeydown={(e) => e.stopPropagation()} role="dialog" aria-labelledby="order-details-title" tabindex="-1">
				<div class="modal-header">
					<div>
						<h2 id="order-details-title" class="modal-title">{selectedOrder.id}</h2>
						<span class="modal-sub">{selectedOrder.customer}</span>
					</div>
					<button class="btn-close" onclick={() => selectedOrder = null} aria-label="Close dialog">
						<X size={18} />
					</button>
				</div>
				<div class="modal-body">
					<div class="detail-grid">
						<div class="detail-item"><span class="d-lbl">Order Date</span><span class="d-val">{selectedOrder.date}</span></div>
						<div class="detail-item"><span class="d-lbl">Total Amount</span><span class="d-val text-accent font-bold">{formatCurrency(selectedOrder.amount)}</span></div>
						<div class="detail-item"><span class="d-lbl">Status</span><span class="badge badge-{statusColors[selectedOrder.status]}">{selectedOrder.status}</span></div>
						<div class="detail-item"><span class="d-lbl">Payment</span><span class="badge badge-{paymentColors[selectedOrder.payment]}">{selectedOrder.payment}</span></div>
					</div>
				...
				<div class="modal-footer">
					...
					<button class="btn-primary" onclick={() => selectedOrder = null}>
						Done
					</button>
				</div>
			</div>
	...
	{#if showCreateModal}
		<div class="modal-backdrop" onclick={() => showCreateModal = false} role="presentation">
			<div class="modal-card" onclick={(e) => e.stopPropagation()} onkeydown={(e) => e.stopPropagation()} role="dialog" aria-labelledby="create-order-title" tabindex="-1">
				<div class="modal-header">
				...
				<form onsubmit={handleCreateOrder} class="modal-form">
					<div class="form-group">
						...
						<input id="c-customer" type="text" placeholder="e.g. Rahman Textiles Ltd." bind:value={newCustomer} required />
					</div>
					<div class="form-row">
						<div class="form-group">
							<label for="c-amount">Total Amount (BDT)</label>
							<input id="c-amount" type="number" placeholder="25000" bind:value={newAmount} required min="1" />
						</div>
						<div class="form-group">
							<label for="c-items">Items Count</label>
							<input id="c-items" type="number" placeholder="5" bind:value={newItems} required min="1" />
						</div>
					</div>
					<div class="form-row">
						<div class="form-group">
					...
					<div class="form-group">
						...
						<input id="c-address" type="text" placeholder="e.g. Banani, Dhaka" bind:value={newAddress} />
					</div>
					<div class="modal-footer">
...
<style>
	...
	.btn-create { display: inline-flex; align-items: center; gap: 6px; padding: 8px 16px; border-radius: var(--radius-md); font-size: 0.8125rem; font-weight: 600; cursor: pointer; background: var(--color-accent); color: white; border: none; }
	.btn-primary { display: inline-flex; align-items: center; gap: 6px; padding: 8px 16px; border-radius: var(--radius-md); font-size: 0.8125rem; font-weight: 600; cursor: pointer; background: var(--color-accent); color: white; border: none; }
	...
	.badge { display: inline-flex; font-size: 0.6875rem; font-weight: 600; padding: 2px 8px; border-radius: 999px; text-transform: capitalize; }
	...
	.modal-sub { font-size: 0.75rem; color: var(--color-text-secondary); }
	...
	.detail-box { background: var(--color-bg-primary); padding: 12px; border-radius: var(--radius-md); border: 1px solid var(--color-border); }
	.d-address { font-size: 0.8125rem; color: var(--color-text-primary); margin: 4px 0 0; }
