		...
		{#each growthMetrics as m, i}
			<div class="growth-card animate-fade-in-up stagger-{i + 1}">
				<div class="growth-top">
					...
					{#if m.trend === 'up'}<TrendingUp size={14} class="trend-up" />{:else}<TrendingDown
							size={14}
							class="trend-down"
						/>{/if}
				</div>
				...
				{#each revenueMonths as m}
					<div class="comp-group">
						<div class="comp-bars">
							<div
								class="comp-bar prev"
								style="height: {(m.prevYear / maxRev) * 100}%"
								title="2025: {fmt(m.prevYear)}"
							></div>
							<div
								class="comp-bar current"
								style="height: {(m.revenue / maxRev) * 100}%"
								title="2026: {fmt(m.revenue)}"
							></div>
						</div>
						<span class="comp-label">{m.month.split(' ')[0]}</span>
						<span
							class="comp-growth"
							class:positive={m.revenue > m.prevYear}
							class:negative={m.revenue <= m.prevYear}
						>
							{m.revenue > m.prevYear ? '+' : ''}{(
								((m.revenue - m.prevYear) / m.prevYear) *
								100
							).toFixed(0)}%
						</span>
					</div>
					...
					<div class="cat-item">
						...
						<div class="cat-bar-area">
							<div class="cat-bar-bg">
								<div
									class="cat-bar-fill"
									style="width: {(cat.revenue / maxCat) * 100}%; background: {cat.color}"
								></div>
							</div>
						</div>
...
<style>
	...
	.comp-growth.positive {
		color: var(--color-success);
	}
	.comp-growth.negative {
		color: var(--color-danger);
	}
