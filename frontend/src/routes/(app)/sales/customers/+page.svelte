...
<script lang="ts">
	...
	let customerList = $state<Customer[]>([
		{
			...
			contact: 'Mr. Abdur Rahman',
			email: 'rahman@textiles.com.bd',
		...
		},
		{
			...
			contact: 'Ms. Fatima Akter',
			email: 'fatima@deh.com.bd',
		...
		},
		{
			...
			contact: 'Mr. Kamal Hossain',
			email: 'kamal@ctgspice.com',
		...
		},
		{
			...
			contact: 'Mr. Rajan Das',
			email: 'rajan@sylhettea.com',
		...
		{
			id: 5,
			name: 'Rajshahi Mangoes Inc.',
			contact: 'Ms. Nusrat Jahan',
			email: 'nusrat@rajmango.com',
		...
		{
			id: 6,
			name: 'Khulna Fisheries',
			contact: 'Mr. Habib Sarker',
			email: 'habib@khulnafish.com',
		...
		{
			id: 7,
			name: 'Narayanganj Jute Works',
			contact: 'Mr. Rafiq Islam',
			email: 'rafiq@juteworks.com',
		...
		{
			id: 8,
			name: 'Comilla Ceramics',
			contact: 'Ms. Ayesha Siddiqui',
			email: 'ayesha@ceramics.com',
		...
		},
		{
			...
			contact: 'Mr. Masud Khan',
			email: 'masud@bogurasteel.com',
		...
		{
			id: 10,
			name: 'Gazipur Garments Ltd.',
			contact: 'Ms. Shamima Begum',
			email: 'shamima@garments.com',
		...
		{
			id: 11,
			name: 'Jessore Food Products',
			contact: 'Mr. Alamgir Hasan',
			email: 'alamgir@jessorefood.com',
		...
		{
			id: 12,
			name: 'Tangail Handicrafts',
			contact: 'Ms. Ruma Khatun',
			email: 'ruma@handicrafts.com',
	...
	const segments = $derived([
		{
			name: 'Enterprise',
			count: customerList.filter((c) => c.segment === 'Enterprise').length,
			revenue: customerList
				.filter((c) => c.segment === 'Enterprise')
				.reduce((a, b) => a + b.ltv, 0),
			color: 'accent'
		},
		{
			name: 'SME',
			count: customerList.filter((c) => c.segment === 'SME').length,
			revenue: customerList.filter((c) => c.segment === 'SME').reduce((a, b) => a + b.ltv, 0),
			color: 'success'
		},
		{
			name: 'Startup',
			count: customerList.filter((c) => c.segment === 'Startup').length,
			revenue: customerList.filter((c) => c.segment === 'Startup').reduce((a, b) => a + b.ltv, 0),
			color: 'warning'
		}
	]);
	...
	<header class="page-header animate-fade-in">
		<div>
			...
			<p class="page-subtitle">
				Track accounts, business contacts, and lifetime value across Bangladesh
			</p>
		</div>
				...
				<tbody>
					{#each paginated as c}
						<tr>
							...
							<td
								><span class="badge badge-{c.status === 'active' ? 'success' : 'neutral'}"
									>{c.status}</span
								></td
							>
							<td>
		...
		<div class="pagination">
			<span class="page-info"
				>Showing {(currentPage - 1) * perPage + 1}–{Math.min(
					currentPage * perPage,
					filtered.length
				)} of {filtered.length} customers</span
			>
			...
			>
				<div class="modal-header">
					<div>
						...
						<span class="badge badge-{segColors[selectedCustomer.segment]}"
							>{selectedCustomer.segment} Tier</span
						>
					</div>
					<button
						class="btn-close"
				...
				<div class="modal-body">
					<div class="detail-grid">
						<div class="detail-item">
							<span class="d-lbl">Contact Person</span><span class="d-val"
								>{selectedCustomer.contact}</span
							>
						</div>
						<div class="detail-item">
							<span class="d-lbl">Email Address</span><span class="d-val"
								>{selectedCustomer.email}</span
							>
						</div>
						<div class="detail-item">
							<span class="d-lbl">Phone</span><span class="d-val">{selectedCustomer.phone}</span>
						</div>
						<div class="detail-item">
							<span class="d-lbl">Region / City</span><span class="d-val"
								>{selectedCustomer.city || 'Dhaka'}</span
							>
						</div>
						<div class="detail-item">
							<span class="d-lbl">Lifetime Spend</span><span class="d-val text-accent font-bold"
								>{fmt(selectedCustomer.ltv)}</span
							>
						</div>
						<div class="detail-item">
							<span class="d-lbl">Completed Orders</span><span class="d-val"
								>{selectedCustomer.orders} orders</span
							>
						</div>
					</div>
				</div>
				<div class="modal-footer">
					<button class="btn-primary" onclick={() => (selectedCustomer = null)}> Done </button>
				</div>
			</div>
		</div>
	{/if}
					...
					<div class="form-row">
						<div class="form-group">
							...
							<input
								id="ac-email"
								type="email"
								placeholder="contact@company.bd"
								bind:value={newEmail}
							/>
						</div>
						<div class="form-group">
