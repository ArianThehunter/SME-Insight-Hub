...
-->
<script lang="ts">
	import { Calendar, Download, FileText, Printer, CheckCircle, ChevronLeft, ChevronRight } from '@lucide/svelte';

	let selectedMonth = $state('July 2026');

	const summaryCards = [
		{ title: 'Total Revenue', value: '৳ 32,45,000', change: '+11.4% vs last month', positive: true },
		{ title: 'Operating Expenses', value: '৳ 19,80,000', change: '+4.2% vs last month', positive: false },
		{ title: 'Net Profit', value: '৳ 12,65,000', change: '+24.1% vs last month', positive: true },
		{ title: 'Tax / VAT Provision', value: '৳ 4,86,750', change: '15% standard rate', positive: true },
	];
...
<div class="page animate-fade-in">
	<header class="page-header">
		<div>
			...
			<p class="page-subtitle">Monthly financial statements, performance reviews, and compliance records</p>
		</div>
...
<style>
	...
	.month-selector { display: flex; justify-content: space-between; align-items: center; padding: 14px 18px; margin: 20px 0 16px; }
	.selector-content { display: flex; align-items: center; gap: 12px; }
	.selector-icon { color: var(--color-accent); }
	.active-month { font-size: 0.9375rem; font-weight: 600; color: var(--color-text-primary); display: block; }
	.status-indicator { font-size: 0.6875rem; color: var(--color-success); display: inline-flex; align-items: center; gap: 4px; font-weight: 500; }
	.select-month { padding: 6px 12px; background: var(--color-bg-primary); border: 1px solid var(--color-border); border-radius: var(--radius-md); color: var(--color-text-primary); font-size: 0.8125rem; }

	.metrics-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 14px; margin-bottom: 20px; }
	.metric-card { padding: 16px; display: flex; flex-direction: column; gap: 4px; }
	.metric-title { font-size: 0.75rem; font-weight: 600; color: var(--color-text-secondary); text-transform: uppercase; }
	.metric-val { font-size: 1.375rem; font-weight: 700; color: var(--color-text-primary); }
	.metric-change { font-size: 0.6875rem; color: var(--color-text-tertiary); }
	.metric-change.positive { color: var(--color-success); font-weight: 600; }
	...
	.data-table th { text-align: left; padding: 10px 18px; font-size: 0.6875rem; color: var(--color-text-tertiary); text-transform: uppercase; border-bottom: 1px solid var(--color-border); }
	...
	.font-bold { font-weight: 700; color: var(--color-success); }
	.table-link { color: var(--color-accent); font-weight: 600; text-decoration: none; font-size: 0.75rem; }
</style>
