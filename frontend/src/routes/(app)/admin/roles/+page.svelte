	...
	const systemRoles = [
		{
			...
			slug: 'org_owner',
			members: 1,
			desc: 'Unrestricted administrative access to all organizational resources, billing, and API tokens.',
			isSystem: true
		},
		{
			...
			slug: 'manager',
			members: 2,
			desc: 'Operational oversight across sales, inventory, finance approvals, and reporting.',
			isSystem: true
		},
		{
			name: 'Accountant',
			slug: 'accountant',
			members: 3,
			desc: 'Full finance & tax management, expense logging, invoice creation, and bank statement reconciliations.',
			isSystem: true
		},
		{
			name: 'Sales Officer',
			slug: 'sales_officer',
			members: 6,
			desc: 'Customer pipeline management, quotation generation, and order processing.',
			isSystem: true
		},
		{
			...
			members: 4,
			desc: 'Warehouse stock movement, purchase order receiving, and low stock threshold alerts.',
			isSystem: true
		},
		{
			name: 'Auditor / Viewer',
			slug: 'viewer',
			members: 2,
			desc: 'Read-only visibility into financial and compliance reports.',
			isSystem: true
		}
	];
</script>
