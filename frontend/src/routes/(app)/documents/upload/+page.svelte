...
<script lang="ts">
	import {
		Upload, FileText, File as FileIcon, Image, Table2, Download,
	...
	const templates = [
		...
		{ type: 'products', label: 'Products', icon: '📦', desc: 'Product catalog with SKU, price, stock' },
		...
		{ type: 'invoices', label: 'Invoices', icon: '🧾', desc: 'Customer invoices and payment status' },
...
<svelte:head>
	...
	<meta name="description" content="Upload PDF invoices, receipts, and bank statements for AI extraction. Or import your data in bulk via CSV templates." />
</svelte:head>
...
<div class="page animate-fade-in">
	<header class="page-header">
		<div>
			...
			<p class="page-subtitle">Upload invoices, receipts, or bank statements for AI extraction — or import bulk data via CSV</p>
		</div>
	</header>
	...
	<div class="upload-grid">
			...
			{#if files.length > 0}
				<div class="file-list">
					{#each files as file (file.id)}
						...
						<div class="file-item" class:success={file.status === 'success'} class:error={file.status === 'error'}>
							<div class="file-icon">
								<DocIcon size={18} />
							</div>
							<div class="file-info">
							...
							<button class="remove-btn" onclick={() => removeFile(file.id)} aria-label="Remove file">
								<X size={14} />
							</button>
						</div>
					{/each}
		...
		<div class="templates-panel">
			<div class="card">
				...
				<p class="section-desc">Download a template, fill it with your data, then upload it as a CSV or Excel file.</p>
				...
				<div class="template-list">
					{#each templates as tmpl}
						<div class="template-item">
							...
							<button class="tmpl-btn" onclick={() => downloadTemplate(tmpl.type)} title="Download {tmpl.label} template">
								<Download size={14} />
								<span>Template</span>
							</button>
						</div>
				...
				<div class="import-note">
					<AlertCircle size={14} />
					<p>After downloading the template, fill it with your data and upload it using the uploader on the left. Select the matching document type.</p>
				</div>
			</div>
...
<style>
	...
	.section-title { font-size: 0.9375rem; font-weight: 600; color: var(--color-text-primary); margin: 0 0 4px; }
	.section-desc { font-size: 0.8125rem; color: var(--color-text-secondary); margin: 0 0 16px; }

	/* ── Doc type selector ────────── */
	.doc-type-row { margin-bottom: 16px; }
	.field-label { font-size: 0.75rem; font-weight: 600; color: var(--color-text-secondary); display: block; margin-bottom: 6px; text-transform: uppercase; letter-spacing: 0.05em; }
	.type-select {
		width: 100%;
		padding: 8px 12px;
	...
	.drop-icon { color: var(--color-accent); opacity: 0.7; margin-bottom: 4px; }
	.drop-title { font-size: 0.9375rem; font-weight: 600; color: var(--color-text-primary); margin: 0; }
	.drop-link { color: var(--color-accent); }
	.drop-hint { font-size: 0.75rem; color: var(--color-text-tertiary); margin: 0; }
	...
	.file-item.success { border-color: var(--color-success); }
	.file-item.error { border-color: var(--color-danger); }

	.file-icon { color: var(--color-text-secondary); flex-shrink: 0; }
	.file-info { flex: 1; min-width: 0; }
	.file-name { font-size: 0.8125rem; font-weight: 500; color: var(--color-text-primary); display: block; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
	.file-meta { font-size: 0.6875rem; color: var(--color-text-tertiary); }
	.file-status { display: flex; align-items: center; gap: 6px; flex-shrink: 0; }

	:global(.text-success) { color: var(--color-success); }
	:global(.text-danger) { color: var(--color-danger); }
	:global(.spinning) { animation: spin 1s linear infinite; }

	.process-link { font-size: 0.75rem; color: var(--color-accent); text-decoration: none; font-weight: 600; }
	.error-msg { font-size: 0.6875rem; color: var(--color-danger); max-width: 120px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
	...
	.tmpl-info { flex: 1; }
	.tmpl-name { font-size: 0.8125rem; font-weight: 600; color: var(--color-text-primary); display: block; }
	.tmpl-desc { font-size: 0.6875rem; color: var(--color-text-tertiary); }
	.tmpl-btn {
		display: flex;
		align-items: center;
		gap: 4px;
	...
	.guidelines-list li strong { color: var(--color-text-primary); }
</style>
