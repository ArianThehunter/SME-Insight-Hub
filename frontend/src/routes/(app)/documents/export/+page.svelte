...
<script lang="ts">
	...
	const exports: ExportItem[] = [
		{
			id: 'customers',
			name: 'Customers',
			description: 'All customer records with contact info and lifetime value',
		...
		},
		{
			id: 'orders',
			name: 'Sales Orders',
			description: 'Orders with line items, status, and payment details',
		...
		},
		{
			...
			description: 'All products with SKU, price, cost, and stock levels',
		...
		},
		{
			id: 'expenses',
			name: 'Expenses',
			description: 'All expense records by category, vendor, and date',
		...
		},
		{
			id: 'invoices',
			name: 'Invoices',
			description: 'All invoices with payment status and amounts',
		...
		},
		{
			id: 'suppliers',
			name: 'Suppliers',
			description: 'Supplier list with contact info and rating',
			endpoint: '/sales/suppliers',
			icon: '🏭',
			rowCount: 87,
		...
		},
		{
			...
			description: 'All OCR-extracted fields from uploaded documents',
			endpoint: '/documents',
			icon: '📄',
			rowCount: 247,
			lastExported: '5 hours ago'
		}
	];
	...
	async function exportData(item: ExportItem) {
		...
		const demoHeaders: Record<string, string> = {
			customers:
				'id,name,email,phone,company,city,segment,lifetime_value_bdt,total_orders,is_active',
		...
		setTimeout(() => {
			exported = { ...exported, [item.id]: false };
		}, 3000);
	}
</script>
...
<div class="page animate-fade-in">
	<header class="page-header">
		<div>
			...
			<p class="page-subtitle">
				Export your business data as CSV or Excel. All amounts in BDT (Taka).
			</p>
		</div>
	</header>
	...
	<div class="export-note card">
		...
		<div>
			...
			<p>
				Exports currently include all records. Date-range filtering will be available once the
