...
<script lang="ts">
	import {
		Building2, Search, Plus, MapPin, Users, Package, Percent,
		ShieldAlert, Download, X, Eye, CheckCircle2
	} from '@lucide/svelte';
	...
	const metrics = $derived({
		...
		avgCapacity: Math.round(warehouseList.reduce((acc, w) => acc + w.capacity, 0) / (warehouseList.length || 1)),
	...
	function exportWarehousesCSV() {
		const headers = 'ID,Warehouse_Name,Location,Manager,Capacity_Pct,Area_SqFt,Total_Items,Status\n';
...
<div class="page animate-fade-in">
	<header class="page-header">
		<div>
			...
			<p class="page-subtitle">Track storage utilization, depot managers, and inventory distribution</p>
		</div>
	...
	<div class="summary-grid">
		...
		<div class="card summary-card">
			...
			<div class="summary-details">
				<span class="summary-val text-{capacityColor(metrics.avgCapacity)}">{metrics.avgCapacity}%</span>
	...
	<div class="toolbar">
		<div class="search-box">
			<Search size={16} />
			<input type="text" bind:value={searchQuery} placeholder="Search facility name, manager, or district..." class="search-input" aria-label="Search warehouses" />
		</div>
		<button class="btn-icon" onclick={exportWarehousesCSV} title="Export CSV" aria-label="Export warehouses CSV">
			<Download size={16} />
		</button>
	</div>
			...
			<div class="card warehouse-card">
				...
				<div class="capacity-section">
					...
					<div class="progress-track">
						<div class="progress-fill bg-{capacityColor(w.capacity)}" style="width: {w.capacity}%"></div>
					</div>
				</div>
	...
	{#if selectedWarehouse}
		<div class="modal-backdrop" onclick={() => selectedWarehouse = null} role="presentation">
			<div class="modal-card" onclick={(e) => e.stopPropagation()} onkeydown={(e) => e.stopPropagation()} role="dialog" aria-labelledby="wh-details-title" tabindex="-1">
				<div class="modal-header">
					...
					<button class="btn-close" onclick={() => selectedWarehouse = null} aria-label="Close dialog">
						<X size={18} />
					</button>
				</div>
				<div class="modal-body">
					<div class="detail-grid">
						<div class="detail-item"><span class="d-lbl">Facility Code</span><span class="d-val">{selectedWarehouse.id}</span></div>
						<div class="detail-item"><span class="d-lbl">Depot Manager</span><span class="d-val">{selectedWarehouse.manager}</span></div>
						<div class="detail-item"><span class="d-lbl">Utilization</span><span class="d-val text-{capacityColor(selectedWarehouse.capacity)} font-bold">{selectedWarehouse.capacity}%</span></div>
						<div class="detail-item"><span class="d-lbl">Floor Area</span><span class="d-val">{selectedWarehouse.area.toLocaleString()} sqft</span></div>
						<div class="detail-item"><span class="d-lbl">Items Stored</span><span class="d-val">{selectedWarehouse.items.toLocaleString()} units</span></div>
						<div class="detail-item"><span class="d-lbl">Operational Status</span><span class="badge badge-{selectedWarehouse.status === 'Active' ? 'success' : 'warning'}">{selectedWarehouse.status}</span></div>
					</div>
				</div>
				<div class="modal-footer">
					<button class="btn-primary" onclick={() => selectedWarehouse = null}>
						Done
					</button>
				</div>
			</div>
	...
	{#if showCreateModal}
		<div class="modal-backdrop" onclick={() => showCreateModal = false} role="presentation">
			<div class="modal-card" onclick={(e) => e.stopPropagation()} onkeydown={(e) => e.stopPropagation()} role="dialog" aria-labelledby="add-wh-title" tabindex="-1">
				<div class="modal-header">
				...
				<form onsubmit={handleAddWarehouse} class="modal-form">
					<div class="form-group">
						...
						<input id="aw-name" type="text" placeholder="e.g. Bogura Regional Logistics Hub" bind:value={newName} required />
					</div>
					<div class="form-group">
						<label for="aw-loc">Location / Address (Bangladesh)</label>
						<input id="aw-loc" type="text" placeholder="e.g. Santahar Road, Bogura" bind:value={newLocation} required />
					</div>
					<div class="form-row">
						<div class="form-group">
							<label for="aw-mgr">Depot Manager Name</label>
							<input id="aw-mgr" type="text" placeholder="e.g. Kamal Hossain" bind:value={newManager} />
						</div>
						<div class="form-group">
							<label for="aw-area">Floor Space (Sq Ft)</label>
							<input id="aw-area" type="number" placeholder="12000" bind:value={newArea} min="100" />
						</div>
					</div>
					<div class="modal-footer">
...
<style>
	...
	.summary-icon-wrapper { width: 42px; height: 42px; border-radius: var(--radius-md); background: color-mix(in srgb, var(--color-accent) 12%, transparent); color: var(--color-accent); display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
	...
	.warehouse-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 16px; }
	.warehouse-card { padding: 18px; display: flex; flex-direction: column; gap: 14px; }
	.warehouse-top { display: flex; justify-content: space-between; align-items: flex-start; gap: 8px; }
	.warehouse-name { font-size: 0.9375rem; font-weight: 700; color: var(--color-text-primary); margin: 0; }
	.warehouse-loc { font-size: 0.6875rem; color: var(--color-text-tertiary); display: inline-flex; align-items: center; gap: 4px; margin-top: 2px; }
	...
	.capacity-labels { display: flex; justify-content: space-between; font-size: 0.75rem; color: var(--color-text-secondary); }
	...
	.warehouse-specs { display: flex; flex-direction: column; gap: 6px; background: var(--color-bg-primary); padding: 10px 12px; border-radius: var(--radius-md); border: 1px solid var(--color-border); font-size: 0.75rem; }
	.spec-row { display: flex; justify-content: space-between; color: var(--color-text-secondary); }
	.spec-row strong { color: var(--color-text-primary); }

	.warehouse-footer { margin-top: auto; padding-top: 10px; border-top: 1px solid var(--color-border); }
