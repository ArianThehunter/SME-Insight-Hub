...
<script lang="ts">
	import {
		AlertOctagon, AlertTriangle, CheckCircle, Search, Settings,
		ArrowUpRight, Plus, ShieldCheck, Download, X, ShoppingCart, CheckCheck
	} from '@lucide/svelte';
	...
	function exportAlertsCSV() {
		const headers = 'SKU,Product_Name,Current_Stock,Reorder_Threshold,Severity,Supplier,Timestamp\n';
...
<div class="page animate-fade-in">
	<header class="page-header">
		<div>
			...
			<p class="page-subtitle">Monitor out-of-stock items, critical safety thresholds, and automated vendor PO triggers</p>
		</div>
	...
	<div class="toolbar">
		<div class="search-box">
			<Search size={16} />
			<input type="text" bind:value={searchQuery} placeholder="Search alerts by SKU or title..." class="search-input" aria-label="Search alerts" />
		</div>
		<div class="toolbar-right">
			...
			<button class="btn-icon" onclick={exportAlertsCSV} title="Export CSV" aria-label="Export stock alerts CSV">
				<Download size={16} />
			</button>
		</div>
	</div>
				...
				<tbody>
					{#each filtered as a}
						<tr>
							...
							<td class="num-cell"><span class="stock-val text-{a.stock === 0 ? 'danger' : 'warning'} font-bold">{a.stock} units</span></td>
							...
							<td>
								<div class="btn-row">
									{#if a.severity !== 'resolved'}
										<button class="btn-action-accent" onclick={() => triggerReorder(a.sku, a.supplier)} title="Draft PO">
											<ShoppingCart size={13} /> Reorder
										</button>
										<button class="btn-action" onclick={() => resolveAlert(a.sku)} title="Mark as resolved">
											<CheckCircle size={13} />
										</button>
									{:else}
	...
	{#if showRuleModal}
		<div class="modal-backdrop" onclick={() => showRuleModal = false} role="presentation">
			<div class="modal-card" onclick={(e) => e.stopPropagation()} onkeydown={(e) => e.stopPropagation()} role="dialog" aria-labelledby="add-rule-title" tabindex="-1">
				<div class="modal-header">
					<h2 id="add-rule-title" class="modal-title">Define Stock Alert Rule</h2>
					<button class="btn-close" onclick={() => showRuleModal = false} aria-label="Close dialog">
						<X size={18} />
					</button>
				</div>
				<form onsubmit={handleCreateRule} class="modal-form">
					<div class="form-group">
						...
						<input id="ar-name" type="text" placeholder="e.g. Basmati Rice 5kg" bind:value={newRuleName} required />
					</div>
					<div class="form-row">
						<div class="form-group">
							<label for="ar-sku">SKU Code</label>
							<input id="ar-sku" type="text" placeholder="PRD-019" bind:value={newRuleSKU} required />
						</div>
						<div class="form-group">
							<label for="ar-thresh">Threshold Level</label>
							<input id="ar-thresh" type="number" placeholder="25" bind:value={newThreshold} min="1" required />
						</div>
					</div>
					<div class="form-group">
					...
					<div class="modal-footer">
						<button type="button" class="btn-secondary" onclick={() => showRuleModal = false}>Cancel</button>
...
<style>
	...
	.toast-banner { display: flex; align-items: center; gap: 10px; padding: 12px 18px; background: color-mix(in srgb, var(--color-success) 12%, transparent); border-color: var(--color-success); color: var(--color-success); font-weight: 600; font-size: 0.8125rem; }
	...
	.summary-val { font-size: 1.5rem; font-weight: 700; font-variant-numeric: tabular-nums; }
	...
	.badge-critical { background: color-mix(in srgb, var(--color-danger) 15%, transparent); color: var(--color-danger); }
	.badge-warning { background: color-mix(in srgb, #f59e0b 15%, transparent); color: #d97706; }
	.badge-resolved { background: color-mix(in srgb, var(--color-success) 15%, transparent); color: var(--color-success); }

	.btn-row { display: flex; align-items: center; gap: 6px; }
	.btn-action-accent { display: inline-flex; align-items: center; gap: 4px; padding: 4px 10px; background: var(--color-accent); color: white; border: none; border-radius: var(--radius-sm); font-size: 0.75rem; font-weight: 600; cursor: pointer; }
	.btn-action { display: inline-flex; align-items: center; padding: 4px 8px; background: var(--color-bg-primary); border: 1px solid var(--color-border); border-radius: var(--radius-sm); font-size: 0.75rem; color: var(--color-text-secondary); cursor: pointer; }
	.btn-action:hover { border-color: var(--color-success); color: var(--color-success); }
