...
<script lang="ts">
	...
	let supplierList = $state<SupplierItem[]>([
		{
			id: 'SPL-001',
			name: 'Bengal Agro Ltd',
			category: 'Food & Beverages',
			contactName: 'Mizanur Rahman',
			email: 'mizan@bengalagro.com',
		...
		{
			id: 'SPL-002',
			name: 'Chittagong Textiles Co.',
			category: 'Textiles',
			contactName: 'Farhana Kabir',
			email: 'fkabir@ctgtextiles.com',
		...
		{
			id: 'SPL-003',
			name: 'Dhaka Leather Craft',
			category: 'Handicrafts',
			contactName: 'Sajidul Islam',
			email: 'sajid@dhakaleather.com',
			phone: '+880-1911-889900',
			rating: 4.2,
			onTime: 88,
			activeOrders: 2,
			status: 'Active',
			city: 'Hazaribagh, Dhaka'
		},
		{
			id: 'SPL-004',
			name: 'Sylhet Tea Estates',
			category: 'Food & Beverages',
			contactName: 'Tanzeem Ahmed',
			email: 't.ahmed@sylhettea.com',
			phone: '+880-1311-112233',
			rating: 4.9,
			onTime: 99,
			activeOrders: 0,
			status: 'Preferred',
		...
		{
			id: 'SPL-005',
			name: 'Rajshahi Fruit Growers',
			category: 'Food & Beverages',
			contactName: 'Asaduzzaman',
			email: 'asad@rajshahifruit.com',
	...
	const metrics = $derived({
		...
		avgRating: (
			supplierList.reduce((acc, s) => acc + s.rating, 0) / (supplierList.length || 1)
		).toFixed(1),
		avgOnTime: Math.round(
			supplierList.reduce((acc, s) => acc + s.onTime, 0) / (supplierList.length || 1)
		),
	...
	function exportSuppliersCSV() {
		const headers =
			'Supplier_ID,Company_Name,Category,Contact_Person,Email,Phone,Rating,On_Time_Pct,City,Status\n';
		const rows = filtered
			.map(
				(s) =>
	...
	<header class="page-header">
		<div>
			...
			<p class="page-subtitle">
				Manage supplier directory, reliability ratings, and procurement statistics
			</p>
		</div>
		...
		{#each filtered as s}
			<div class="card supplier-card">
				<div class="supplier-top">
					...
					<span
						class="badge badge-{s.status === 'Preferred'
							? 'success'
							: s.status === 'Active'
								? 'accent'
								: 'warning'}">{s.status}</span
					>
				</div>
	...
	{#if selectedSupplier}
		<div class="modal-backdrop" onclick={() => (selectedSupplier = null)} role="presentation">
			<div
				...
				role="dialog"
				aria-labelledby="spl-details-title"
				tabindex="-1"
			>
					...
					<div class="detail-grid">
						<div class="detail-item">
							<span class="d-lbl">Contact Person</span><span class="d-val"
								>{selectedSupplier.contactName}</span
							>
						</div>
						<div class="detail-item">
							<span class="d-lbl">Phone</span><span class="d-val">{selectedSupplier.phone}</span>
						</div>
						<div class="detail-item">
							<span class="d-lbl">Email</span><span class="d-val">{selectedSupplier.email}</span>
						</div>
						<div class="detail-item">
							<span class="d-lbl">Location / City</span><span class="d-val"
								>{selectedSupplier.city || 'Dhaka'}</span
							>
						</div>
						<div class="detail-item">
							<span class="d-lbl">Vendor Rating</span><span class="d-val text-accent font-bold"
								>★ {selectedSupplier.rating} / 5.0</span
							>
						</div>
						<div class="detail-item">
							<span class="d-lbl">On-Time Reliability</span><span
								class="d-val text-success font-bold">{selectedSupplier.onTime}%</span
							>
						</div>
					</div>
				</div>
				<div class="modal-footer">
					<button class="btn-primary" onclick={() => (selectedSupplier = null)}> Done </button>
				</div>
			</div>
		</div>
	{/if}
				...
				<form onsubmit={handleAddSupplier} class="modal-form">
					<div class="form-group">
						...
						<input
							id="as-name"
							type="text"
							placeholder="e.g. Chittagong Steel Suppliers"
							bind:value={newName}
							required
						/>
					</div>
					...
					<div class="form-row">
						<div class="form-group">
							...
							<input
								id="as-contact"
								type="text"
								placeholder="e.g. Mr. Rafiqul Bari"
