...
<script lang="ts">
	import {
		Search, Plus, Download, Package, AlertTriangle, Eye,
		X, CheckCircle2, DollarSign, Layers
	} from '@lucide/svelte';
	...
	let productList = $state<ProductItem[]>([
		{ sku: 'PRD-001', name: 'Premium Basmati Rice 5kg', category: 'Food & Beverages', price: 850, cost: 620, stock: 342, reorder: 100, status: 'In Stock', unit: 'bag' },
		{ sku: 'PRD-002', name: 'Organic Mustard Oil 1L', category: 'Food & Beverages', price: 320, cost: 210, stock: 580, reorder: 200, status: 'In Stock', unit: 'bottle' },
		{ sku: 'PRD-003', name: 'Bengal Spice Mix Set', category: 'Food & Beverages', price: 450, cost: 280, stock: 15, reorder: 50, status: 'Low Stock', unit: 'pack' },
		{ sku: 'PRD-004', name: 'Handloom Cotton Saree', category: 'Textiles', price: 3200, cost: 1800, stock: 78, reorder: 30, status: 'In Stock', unit: 'pcs' },
		{ sku: 'PRD-005', name: 'Artisan Leather Bag', category: 'Handicrafts', price: 4500, cost: 2800, stock: 0, reorder: 15, status: 'Out of Stock', unit: 'pcs' },
		{ sku: 'PRD-006', name: 'Jamdani Silk Fabric (1m)', category: 'Textiles', price: 2800, cost: 1650, stock: 120, reorder: 40, status: 'In Stock', unit: 'meter' },
		{ sku: 'PRD-007', name: 'Pabna Yogurt 500g', category: 'Food & Beverages', price: 180, cost: 110, stock: 8, reorder: 100, status: 'Low Stock', unit: 'pot' },
		{ sku: 'PRD-008', name: 'Brass Decorative Plate', category: 'Handicrafts', price: 1500, cost: 850, stock: 45, reorder: 20, status: 'In Stock', unit: 'pcs' },
		{ sku: 'PRD-009', name: 'Sylhet Orange Tea 250g', category: 'Food & Beverages', price: 650, cost: 380, stock: 234, reorder: 80, status: 'In Stock', unit: 'box' },
		{ sku: 'PRD-010', name: 'Jute Tote Bag (Eco)', category: 'Handicrafts', price: 550, cost: 280, stock: 0, reorder: 50, status: 'Out of Stock', unit: 'pcs' },
		{ sku: 'PRD-011', name: 'Nakshi Kantha Bedcover', category: 'Textiles', price: 5200, cost: 3100, stock: 22, reorder: 10, status: 'In Stock', unit: 'pcs' },
		{ sku: 'PRD-012', name: 'Mango Pickle Jar 500g', category: 'Food & Beverages', price: 220, cost: 120, stock: 456, reorder: 150, status: 'In Stock', unit: 'jar' },
	]);
...
<div class="page">
	<header class="page-header animate-fade-in">
		<div>
			...
			<p class="page-subtitle">Track SKUs, retail pricing, gross margins, and safety stock levels</p>
		</div>
	...
	<div class="toolbar">
		<div class="search-box">
			<Search size={16} />
			<input type="text" bind:value={searchQuery} placeholder="Search product name or SKU..." class="search-input" aria-label="Search products" />
		</div>
		<div class="toolbar-right">
			...
			<button class="btn-icon" onclick={exportProductsCSV} title="Export CSV" aria-label="Export products catalog">
				<Download size={16} />
			</button>
		</div>
	</div>
				...
				<tbody>
					{#each filtered as p}
						...
						<tr>
							...
							<td class="num-cell"><strong>{p.stock}</strong> <span class="unit-text">{p.unit || 'pcs'}</span></td>
							<td>
								<span class="badge badge-{p.status === 'In Stock' ? 'success' : p.status === 'Low Stock' ? 'warning' : 'danger'}">
									{p.status}
								</span>
							</td>
							<td>
								<button class="btn-icon-row" onclick={() => selectedProduct = p} title="View product details">
									<Eye size={14} />
								</button>
							</td>
						</tr>
	...
	{#if selectedProduct}
		{@const marginPct = Math.round(((selectedProduct.price - selectedProduct.cost) / selectedProduct.price) * 100)}
		<div class="modal-backdrop" onclick={() => selectedProduct = null} role="presentation">
			<div class="modal-card" onclick={(e) => e.stopPropagation()} onkeydown={(e) => e.stopPropagation()} role="dialog" aria-labelledby="prod-details-title" tabindex="-1">
				<div class="modal-header">
					<div>
						<h2 id="prod-details-title" class="modal-title">{selectedProduct.name}</h2>
						<span class="sku-tag">{selectedProduct.sku}</span>
					</div>
					<button class="btn-close" onclick={() => selectedProduct = null} aria-label="Close dialog">
						<X size={18} />
					</button>
				</div>
				<div class="modal-body">
					<div class="detail-grid">
						<div class="detail-item"><span class="d-lbl">Category</span><span class="d-val">{selectedProduct.category}</span></div>
						<div class="detail-item"><span class="d-lbl">Stock Quantity</span><span class="d-val">{selectedProduct.stock} {selectedProduct.unit || 'pcs'}</span></div>
						<div class="detail-item"><span class="d-lbl">Selling Price</span><span class="d-val text-accent font-bold">৳ {selectedProduct.price.toLocaleString()}</span></div>
						<div class="detail-item"><span class="d-lbl">Unit Cost</span><span class="d-val">৳ {selectedProduct.cost.toLocaleString()}</span></div>
						<div class="detail-item"><span class="d-lbl">Gross Margin</span><span class="d-val text-success font-bold">{marginPct}%</span></div>
						<div class="detail-item"><span class="d-lbl">Reorder Safety Level</span><span class="d-val">{selectedProduct.reorder} units</span></div>
					</div>
				</div>
				<div class="modal-footer">
					<button class="btn-primary" onclick={() => selectedProduct = null}>
						Done
					</button>
				</div>
			</div>
	...
	{#if showCreateModal}
		<div class="modal-backdrop" onclick={() => showCreateModal = false} role="presentation">
			<div class="modal-card" onclick={(e) => e.stopPropagation()} onkeydown={(e) => e.stopPropagation()} role="dialog" aria-labelledby="add-prod-title" tabindex="-1">
				<div class="modal-header">
				...
				<form onsubmit={handleAddProduct} class="modal-form">
					<div class="form-group">
						...
						<input id="ap-name" type="text" placeholder="e.g. Silk Dupatta Handwoven" bind:value={newName} required />
					</div>
					<div class="form-row">
						<div class="form-group">
							<label for="ap-cat">Category</label>
							<input id="ap-cat" type="text" placeholder="Textiles" bind:value={newCategory} required />
						</div>
						<div class="form-group">
					...
					<div class="form-row">
						<div class="form-group">
							...
							<input id="ap-price" type="number" placeholder="1200" bind:value={newPrice} required min="1" />
						</div>
						<div class="form-group">
							<label for="ap-cost">Unit Cost (BDT)</label>
							<input id="ap-cost" type="number" placeholder="800" bind:value={newCost} required min="1" />
						</div>
					</div>
					<div class="form-row">
						<div class="form-group">
							<label for="ap-stock">Initial Stock</label>
							<input id="ap-stock" type="number" placeholder="100" bind:value={newStock} required min="0" />
						</div>
						<div class="form-group">
							<label for="ap-reorder">Reorder Threshold</label>
							<input id="ap-reorder" type="number" placeholder="20" bind:value={newReorder} required min="1" />
						</div>
					</div>
					<div class="modal-footer">
...
<style>
	...
	.sum-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 14px; }
	.sum-card { background: var(--color-bg-secondary); border: 1px solid var(--color-border); border-radius: var(--radius-lg); padding: 16px; display: flex; flex-direction: column; gap: 4px; }
	.sum-label { font-size: 0.75rem; font-weight: 600; color: var(--color-text-secondary); text-transform: uppercase; }
	.sum-val { font-size: 1.5rem; font-weight: 700; color: var(--color-text-primary); font-variant-numeric: tabular-nums; }
	.text-success .sum-val { color: var(--color-success); }
	.text-warning .sum-val { color: #d97706; }
	.text-danger .sum-val { color: var(--color-danger); }
	...
	.sku-tag { font-family: monospace; font-size: 0.75rem; background: var(--color-bg-primary); padding: 2px 6px; border-radius: 4px; border: 1px solid var(--color-border); color: var(--color-accent); }
	.cat-pill { font-size: 0.6875rem; padding: 2px 8px; border-radius: 999px; background: var(--color-bg-primary); border: 1px solid var(--color-border); color: var(--color-text-secondary); }
	.unit-text { font-size: 0.6875rem; color: var(--color-text-tertiary); }
